-- ============================================================
-- KRABBIE.COM — Complete Database Schema
-- วิธีรัน: วาง SQL นี้ทั้งหมดใน Supabase SQL Editor แล้วกด Run
-- หรือรัน: psql $DATABASE_URL -f schema.sql
-- ============================================================
...
-- Tenants: ทุกคนอ่านได้ (ลูกค้าต้องเห็นชื่อร้าน)
...
-- Time slots: อ่าน slot ที่ไม่ถูก block
...
-- Bookings: ลูกค้า insert ได้, service role ทำได้ทั้งหมด
...
-- Payments: service role เท่านั้น
...
-- booking confirmed/completed timestamps
...
-- mark slot booked + race condition guard
...
-- generate slots ล่วงหน้า N วัน ตาม business hours
...
-- cleanup old slots (เรียกโดย cron ทุกอาทิตย์)
...
-- Demo tenant (DEV ONLY — ลบออกก่อน deploy production)
...
-- Generate slots 7 วัน สำหรับ demo tenant
DO $$
