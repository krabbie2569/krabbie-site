      ...
      <div className="max-w-5xl mx-auto px-6 py-8 space-y-8">
        ...
        <div className="card">
          ...
          <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
            <div className="h-full bg-orange-500 rounded-full transition-all" style={{ width: `${Math.min((stats.activeTenants / 6) * 100, 100)}%` }} />
          </div>
        ...
        <div>
          ...
          <div className="bg-white rounded-krabbie border border-krabbie-border overflow-hidden">
            <table className="w-full text-sm">
              ...
              <tbody>
                {list.length === 0 ? (
                  <tr><td colSpan={5} className="text-center py-12 text-gray-400">ยังไม่มีร้านค้า</td></tr>
                ) : list.map((t) => (
                  <tr key={t.id} className="border-b border-gray-50 hover:bg-orange-50/30 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <span className={`text-[0.6rem] font-mono px-2 py-0.5 rounded ${t.plan === 'active' ? 'badge-live' : t.plan === 'trial' ? 'badge-trial' : 'bg-gray-100 text-gray-400'}`}>
                        {TENANT_PLAN_LABEL[t.plan]}
                      </span>
                    </td>
                    <td className="px-4 py-3 font-mono text-xs text-gray-400">{new Date(t.created_at).toLocaleDateString('th-TH')}</td>
                  </tr>
                ))}
              </tbody>
            </table>
